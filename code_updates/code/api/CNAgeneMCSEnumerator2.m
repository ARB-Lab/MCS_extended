...
%       options.compression_network_pre_GPR  : <logical> Network compression (prior to GPR rule integration).
%                                                        (default FALSE)
...
if isa(options,'struct')
    if ~isfield(options,'compression_network_pre_GPR')
...
if nargin < 6 || isempty(koCost) % if no reaction KO costs are defined, assume no reactions but all genes are knockable
    koCost = nan(1,cnap.numr);
    koCost([gpr_rules(:).reaction]) = 1;
end
if nargin < 10 || isempty(gkoCost) % derive from reaction knock-outs, if not defined. Set reactions with gene-association to notknockable.
    ...
    for i = 1:length(genes) % if all associated reactions are notknockable, so is the gene.
        ...
        if all(isnan(koCost([gpr_rules(rules_i).reaction])))
            gkoCost(i) = nan;
        end
    end
else
...
if nargin < 11 || isempty(gkiCost) % derive from reaction knock-ins, if not defined
    ...
    for i = 1:length(genes)
        rules_i = cellfun(@(x) ismember(i,x),{gpr_rules(:).genes});  % bool vector: Rules that contain the gene
        if ~all(isnan(kiCost([gpr_rules(rules_i).reaction])))
            gkiCost(i) = min(kiCost([gpr_rules(rules_i).reaction]));
        end
    end
else
    gkiCost = gkiCost(:)';
...
kiCost = kiCost(:)';
kiCost([gpr_rules(:).reaction]) = nan;  % gene-associations 'override' reaction knock-outs
koCost = koCost(:)';
koCost([gpr_rules(:).reaction]) = nan;
...
[reacMin, reacMax] = CNAfluxVariability(cnap,[],c_macro,2);
...
for i = 1:length(D)
    [lb_D, ub_D] = CNAfluxVariability(cnap,[],c_macro,2,1:cnap.numr,D{i},d{i}); %#ok<*AGROW> Allow change of size in each loop iteration
...
if options.compression_network_pre_GPR
%% compress network if indicated
    displ('Compressing GEM model.',verbose);
    [        precmp_cnap, T, D, koCost, kiCost, cmp_transf_mat, gkoCost, gkiCost, gpr_rules ] = ... Compression
    compress(cnap, T, D, koCost, kiCost,           reac_off, gkoCost, gkiCost, gpr_rules);
    text = 'compressed ';
else
    text = '';
...
if ~isempty(gpr_rules)
    [ precmp_cnap, M_GEM_map, ~,koCost, kiCost, T, D, rType ] = ...
    CNAintegrateGPRrules( precmp_cnap, gpr_rules, koCost, kiCost, T, D, gkoCost, gkiCost, 0);
else
    M_GEM_map = eye(precmp_cnap.numr);
    rType = repmat('r',1,precmp_cnap.numr);
    cnap.rType = rType;
end
...
feas_D = testRegionFeas(precmp_cnap,D,d,2);
feas_T = testRegionFeas(precmp_cnap,T,t,2);
if any(~feas_T)
    error(['At least one target region (T' num2str(find(~feas_T)) ') is infeasible in the original model.']);
end
if any(~feas_D)
...
[r_mcs, status, cmp_mcs, cmp_cnap, mcs_idx_cmp, obj] = ...
    CNAMCSEnumerator2(precmp_cnap,T,t,D,d,koCost,kiCost,maxSolutions,maxCost,options,verbose);
...
if status ~= 2 && ~all(all(isnan(r_mcs)))
    ...
    mcs_reac  = sparse(cmp_transf_mat*M_GEM_map*r_mcs_exp);
    ...
    ivCost(isnan(ivCost))    = maxCost+1; % higher number, so that mcs containing this intervention are sortet out
    % check if knock-out and knock-in costs are still met
    mcs_affordable = ivCost*(mcs_expanded~=0 & ~isnan(mcs_expanded)) <= maxCost;
    ...
    rmcs = gmcs2rmcs(full_mcs,gpr_rules_orig,rmap,full_cnap.rType);
else
    switch status
        case 1
